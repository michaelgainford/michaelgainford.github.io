...
export default function Component_StyleGuideInfo() {
	return (
		<div className="grid w-full @4xl:grid-cols-2 gap-8 lg:gap-16 style-guide-main">
			<div className="flex flex-col p-8 text-left border rounded-lg border-slate-200/5 style-guide-group bg-slate-200/5">
				<h2
					className={`${sharedHeadingStyles} text-left mr-auto flex justify-start w-fit`}
				>
					Typography
				</h2>
					...
					<table>
						...
						<tbody>
							{FontSizes.map((fontSize, index) => (
								<tr
									key={index}
									className="grid grid-cols-3 border-b border-slate-200/5 last:border-transparent"
								>
					...
					<table>
						...
						<tbody>
							{CSSContainerWidths.map((containerWidth, index) => (
								<tr
									key={index}
									className="grid grid-cols-3 gap-2 border-b-2 border-slate-200/5 last:border-transparent"
								>
			...
			<div className="p-8 text-left border rounded-lg border-slate-200/5 style-guide-group bg-slate-200/5 @container">
				...
				<div className="grid grid-cols-2 @lg:grid-cols-3 @xl:grid-cols-4 gap-4 style-guide-group-items colour-swatch">
					{BrandColours.map((colour, index) => (
						<div
							key={index}
							className="flex flex-col gap-4 transition-all duration-500 opacity-80 hover:opacity-100"
						>
							<div
								className={`swatch flex items-center justify-center rounded-md colour ${colour.class_background} aspect-square`}
							>
								<p
									className={`${colour.class_background_text} text-xs bg-slate`}
								>
									{colour.class}
								</p>
							</div>
