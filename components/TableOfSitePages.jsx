                    ...
                    {PagesInSitemap.map((item, index) => (
                        <tr
                            ...
                            className={`border-b border-slate-200/5 last:border-transparent [&>th]:px-4 [&>th]:py-2 ${
                                Math.floor((today - new Date(item.lastUpdated)) / (1000 * 60 * 60 * 24)) > daysInWarningZone 
                                ? 'bg-rose-500/10' 
                                : Math.floor((today - new Date(item.lastUpdated)) / (1000 * 60 * 60 * 24)) > daysInAttentionZone
                                ? 'bg-yellow-500/10': ''
                            }`}
                        >
                            ...
                            <span className="px-2 py-1 font-bold w-full tracking-wider text-center rounded-lg relative">
                                <span
                                    ...
                                    Math.floor((today - new Date(item.lastUpdated)) / (1000 * 60 * 60 * 24)) <= daysInSafeZone
                                        ? 'bg-green-500'
                                    : Math.floor((today - new Date(item.lastUpdated)) / (1000 * 60 * 60 * 24)) <= daysInAttentionZone 
                                        ? 'bg-sky-300'
                                    : Math.floor((today - new Date(item.lastUpdated)) / (1000 * 60 * 60 * 24)) <= daysInWarningZone
