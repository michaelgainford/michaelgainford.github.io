...
export default function MatrixHero({ page, stats }) {
  return (
    <section className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
      <div className="rounded-2xl border border-slate-800/80 bg-slate-950/70 p-5 shadow-xl shadow-black/30 lg:p-8">
        <span className="mb-4 inline-flex rounded-full border border-slate-700/80 bg-slate-900/70 px-3 py-1 text-xxs uppercase tracking-[0.18em] text-slate-300">
          Reality Distortion Archive
        </span>
        <h2 className="text-2xl font-semibold uppercase tracking-wide text-slate-100 lg:text-4xl">{page.title}</h2>
        <p className="mt-3 text-sm uppercase tracking-[0.2em] text-slate-400">{page.tagline}</p>
        ...
        <div className="mt-6 grid grid-cols-2 gap-2 lg:grid-cols-4">
          <div className="rounded-md border border-slate-800/80 bg-slate-900/60 p-3 text-center">
            ...
            <p className="mt-1 text-sm font-semibold text-slate-100">{page.releaseYear}</p>
          </div>
          <div className="rounded-md border border-slate-800/80 bg-slate-900/60 p-3 text-center">
            <p className="text-xxs uppercase tracking-[0.14em] text-slate-400">Runtime</p>
            <p className="mt-1 text-sm font-semibold text-slate-100">{page.runtimeMinutes}m</p>
          </div>
          <div className="rounded-md border border-slate-800/80 bg-slate-900/60 p-3 text-center">
            <p className="text-xxs uppercase tracking-[0.14em] text-slate-400">IMDb</p>
            <p className="mt-1 text-sm font-semibold text-slate-100">{page.imdb}</p>
          </div>
          <div className="rounded-md border border-slate-800/80 bg-slate-900/60 p-3 text-center">
            <p className="text-xxs uppercase tracking-[0.14em] text-slate-400">Club Rating</p>
            <p className="mt-1 text-sm font-semibold text-slate-100">{page.clubRating}/10</p>
          </div>
        </div>
      </div>

      <div className="rounded-2xl border border-slate-800/80 bg-slate-950/65 p-4 lg:p-6">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          <div className="rounded-md border border-slate-800/80 bg-slate-900/60 p-3 text-center">
            ...
            <p className="mt-1 text-lg font-semibold text-slate-100">{stats.trilogyLength}</p>
          </div>
          <div className="rounded-md border border-slate-800/80 bg-slate-900/60 p-3 text-center">
            <p className="text-xxs uppercase tracking-[0.14em] text-slate-400">Quotes</p>
            <p className="mt-1 text-lg font-semibold text-slate-100">{stats.quoteCount}</p>
          </div>
          <div className="rounded-md border border-slate-800/80 bg-slate-900/60 p-3 text-center">
            <p className="text-xxs uppercase tracking-[0.14em] text-slate-400">Cast</p>
            <p className="mt-1 text-lg font-semibold text-slate-100">{stats.castLength}</p>
          </div>
        </div>
      </div>
    </section>
