                    ...
                    <div className="relative flex flex-col gap-4 px-4 py-8 border rounded-lg lg:py-12" key={TopScorer.playerId}>
                        ...
                        <div className={`absolute bottom-1 left-[4%] text-[10px] mx-auto w-[92%]`}>
                            <p className="px-4 mx-auto italic text-slate-400 text-balance">
                                Last scored in the Premier League for {TopScorer.playerLastGoalFor} on {new Date(TopScorer.playerLastGoalDate).toLocaleDateString('en-GB')} ({calculateDaysAgo(TopScorer.playerLastGoalDate)} {calculateDaysAgo(TopScorer.playerLastGoalDate) === 1 ? 'day' : 'days'} ago) vs {TopScorer.playerLastGoalOpponent}.
                            </p>
                        </div>
                    </div>
                );
