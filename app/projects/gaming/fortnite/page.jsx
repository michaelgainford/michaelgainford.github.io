  ...
  return (
    <div className={`${globalPageStyles} !bg-topo-background`}>
      ...
      <main className={`${mainStyles}`}>
        <H1 text={`Fortnite`} />
        <div
          className={`grid w-full grid-cols-1 gap-8 lg:gap-16 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols`}
        >
          {DataForFortnite.map((season, index) => (
            <div
              key={index}
              className={`bg-green-900 p-4 lg:py-8 flex flex-col justify-end aspect-[1000/563] lg:aspect-[5/3] relative lg:hover:opacity-100 transition group/season duration-300`}
            >
              <div
                className={`relative z-20 space-y-2 text-slate-400 group-hover/season:hidden group-hover/season:opacity-100 group/content`}
              >
                <p className={`text-base font-medium lg:text-2xl`}>
                  Chapter {season.chapter} Season {season.season}
                </p>
                <p className={`text-xs lg:text-sm uppercase tracking-widest font-bold`}>{season.theme}</p>
