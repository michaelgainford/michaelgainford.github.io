    ...
    <div className={`${globalPageStyles} ${pageStyles}`}>
      ...
      <main className={`${mainStyles}`}>
        ...
        <div className="flex flex-wrap justify-center w-full gap-8 max-w-[80vw]">
          <ul className="grid flex-wrap grid-cols-3 gap-4 lg:gap-8 xl:gap-12">
          {DataForSpace.map((planet, index) => (
            <li key={index} className="w-full p-4 lg:p-8 space-y-2 text-center border rounded-lg border-slate-600 bg-slate-800 min-w-[200px] max-sm:w-full flex flex-col items-center">
              ...
              <span className="grid grid-cols-2 gap-4">
                <span className="flex flex-col tracking-wider text-center">
                  <span className="">{planet.distanceFromSunInMiles.toLocaleString()}</span>
        ...
        <div className="w-full mt-16 solar-system">
          ...
          <ol>
          {DataForSpace.map((planet, index) => (
            <li key={index} className={planet.name.toLowerCase()} data-order={planet.order} data-handle={planet.name.toLowerCase()}>
            </li>
          ))}
          </ol>
        </div>
