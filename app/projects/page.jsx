		...
		<div className={`${globalPageStyles} ${pageStyles}`}>
			...
			<main className={`${mainStyles}`}>
				...
				<div className="grid flex-wrap w-full grid-cols-1 gap-6 xs:grid-cols-2 xs:gap-6 md:gap-8 md:grid-cols-3 lg:grid-cols-4 lg:gap-8 2xl:grid-cols-5 2xl:gap-16 motion motion-preset-slide-up motion-delay-[1000ms] motion-duration-50">
					...
					<Link key={index} href={project.href} className={`border-2 ${project.border} ${project.hover} ${project.color} h-full  rounded-lg space-y-4 gap-4 flex flex-col justify-between bg-transparent ${project.archived ? `opacity-50` : ``} cursor-pointer group`}>
						<span className={`flex justify-center items-center gap-2 lg:gap-4 pt-4 rounded-t-md py-4 ${project.bg}`}>
							...
							<h2 className="text-xs font-light uppercase">{project.name}</h2>
						</span>
						<span className="flex flex-col justify-between gap-4 px-4 pb-6 text-slate-200">
							<span className={`flex justify-center text-xs tracking-wider text-center text-current group-hover:${project.color} text-balance`}>
								<span className="text-center line-clamp-2">{project.description}</span>
							</span>
