...
const pageStyles = "relative overflow-hidden bg-linear-to-br from-[#07111c] via-[#0d2030] to-[#18354a] text-slate-100";
...
export default function Page_TheWorld() {
	return (
		<div className={`${globalPageStyles} ${pageStyles} text-slate-300`}>
			<div className="pointer-events-none absolute inset-x-0 top-0 h-[500px] bg-[radial-gradient(ellipse_at_top,rgba(103,232,249,0.16),transparent_70%)]" />
			<div className="pointer-events-none absolute inset-x-0 -top-32 mx-auto h-[280px] w-[280px] rounded-full bg-sky-300/10 blur-3xl" />
			<Header />
			<main className={`${globalWrapperFixedWidth}`}>
				<section className="w-full max-w-[1700px] rounded-2xl border border-slate-200/12 bg-slate-950/60 p-4 shadow-2xl shadow-slate-950/35 backdrop-blur-sm md:p-6 lg:p-8">
					...
					<div className={`flex flex-col items-center w-full lg:gap-8 xl:gap-12 2xl:gap-16`}>
						<div className={`flex flex-col justify-center w-full gap-8 max-w-[95%] lg:max-w-[96%]`}>
							<PageIntro
								text={pageIntroText}
								text_colour="text-slate-200/90"
								classes_width="max-w-[960px]"
								classes_font_sizes="text-xs/6 lg:text-sm/7"
								classes_margin="!mb-8"
							/>
							<div className="mx-auto mb-2 max-w-[920px] rounded-xl border border-cyan-300/20 bg-cyan-400/10 px-4 py-3 text-center text-xs tracking-wide text-cyan-50/90 lg:text-sm">
