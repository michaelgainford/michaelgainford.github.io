...
export default async function ClubPage({ params }) {
  ...
  return (
    <div className="min-h-screen bg-white text-epl">
      <Header />
      <main className="flex flex-col items-stretch py-12 px-8 w-full border-4 border-pink-500">
				<div className="club-header border-2 border-amber-500 flex flex-col items-center">
					...
					<p className="text-lg mb-4">{club.currentPrem ? "Current Premier League Club" : "Former Premier League Club"}</p>
				</div>
        {/* Add more club details here as needed */}
				<div className="club-information-area px-8 border-4 w-full border-emerald-500">
					<ul>
						...
						<li>Stadium Name: {Array.isArray(club.stadiumDetails) && club.stadiumDetails.length > 0 && club.stadiumDetails[0].stadiumName
								? club.stadiumDetails[0].stadiumName
								: club.stadium}</li>
						<li>Stadium Opened: {Array.isArray(club.stadiumDetails) && club.stadiumDetails.length > 0 && club.stadiumDetails[0].stadiumOpened
								? club.stadiumDetails[0].stadiumOpened
								: "N/A"}</li>
						<li>Capacity: {
							Array.isArray(club.stadiumDetails) && club.stadiumDetails.length > 0 && club.stadiumDetails[0].stadiumCapacity
						...
						<li>Relegations from Premier League: {club.relegationsFromPremierLeague}</li>
