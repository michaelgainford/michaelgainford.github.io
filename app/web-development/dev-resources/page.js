  ...
  return (
    <div className={`${globalPageStyles} bg-graph-paper-background bg-teal-800 text-slate-300`}>
      ...
      <main className={mainStyles}>
         ...
         <div className="flex flex-wrap justify-center hidden gap-2 p-2 resource-tags lg:w-3/4">
         ...
         <div className="grid w-full grid-cols-2 pt-4 sm:grid-cols-3 sm:pt-6 lg:pt-16 resources md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-x-4 gap-y-8 md:gap-8 lg:gap-12">
          ...
          {ResourcesSortedByName.map((resource, index) => (
            <Link target="_blank" href={resource.url} key={index} className="flex flex-col items-center justify-center text-center duration-300 rounded-lg resource border-emerald-950 bg-emerald-950 hover:bg-gradient-to-b hover: hover:from-teal-800 hover:to-teal-950 text-white/80 group aspect-[5/2] drop-shadow-lg">
              <div className="w-full bg-teal-700 rounded-t-lg resource-image aspect-square">
                <Image src={resource.image} alt={resource.name} layout="fill" objectFit="cover" objectPosition="center" className="rounded-t-lg" />
              </div>
              <div className="grid w-full py-2 sm:space-y-4 divide-teal-900 grid-cols-1 sm:divide-y resource-details grid-rows-[40px_1fr] border">
